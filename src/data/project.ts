...
export const PROJECT_CONTENTS: TProject[] = [
  {
    ...
    tasks: [
      '<b>기획 일부 참여 / 디자인 및 프론트엔드 개발 전담</b>',
      '<b>프로젝트 CRUD 및 검색 기능 구현</b>',
      '<b>인재 검색 및 필터링</b><br>직군, 연차, 태그, 자연어의 네 가지 필터 적용 가능<br>각 태그는 url의 query parameter에 저장됨<br>검색 기록 자동 저장',
      '<b>기타 다양한 기능 구현</b><br>검색 기록, 숏리스트 관리<br>인재 신고, 인재 URL 복사, 인재 메모<br>의뢰 요청 기능 구현',
      '<b>사용자 친화적인 코치마크 형태의 사용법 설명 UI 구현</b>',
      '<b>무료 이용 기간 표시 및 무료 이용 종료 여부에 따라 나타나는 인증 모달 구현</b>',
      '<b>데이터의 효율적인 로딩을 위한 페이지네이션 적용</b>',
    ],
    troubles: [
  ...
  },
  {
    ...
    tasks: [
      '<b>블로그 페이지 추가</b><br>DB에 저장된 블로그 글을 보여주는 화면과 기능 개발<br>블로그 제목에 따른 목차(ToC) 인터페이스 개발',
      '<b>무료 리스트 신청 페이지 내재화</b><br>기존에 smore로 구현되어 있던 신청 페이지 내재화<br>유효성 검사 및 에러 처리',
      '<b>SEO 최적화 + 써드파티 서비스 연동</b><br>SSG(static export)로 구현<br>메타데이터, 오픈그래프, 캐노니컬 태그 적용<br>채널톡, Hotjar 연결',
      '<b>AWS 배포 CI/CD</b><br>Cloudfront + S3 세팅<br>린팅, 빌드 후 aws `sync` 명령어를 이용한 배포<br>배포 성공 여부에 따른 슬랙 알림 설정',
      '<b>홈페이지 디자인 리뉴얼</b><br>외주 디자인을 바탕으로 퍼블리싱',
    ],
    troubles: [
  ...
  },
  {
    ...
    tasks: [
      '<b>식단, 운동 조회 및 삭제</b><br>날짜별로 식단과 운동을 조회하는 기능 구현<br>날짜 선택 방식이 불편하다는 사용자 피드백<br>→ swiper를 이용하여 캘린더 날짜 전환 방식을 드래그로 변경',
      '<b>식단, 운동 기록 및 수정</b><br>날짜와 끼니 별로 하나의 식단을 등록 및 수정 가능하게 구현<br>낙관적 업데이트를 적용하여 운동 완료 체크 속도 1.2초 -> 0.01초로 개선<br>음식의 칼로리 입력에 어려움을 겪는다는 유저 테스트 결과<br>→ debounce를 활용한 음식 영양 성분 검색 기능 추가<br>코드의 유지보수성과 가독성을 개선하기 위해 약 50줄의 주요 로직을 커스텀 훅으로 분리',
      '<b>캘린더, 로딩 컴포넌트 구현 및 UI 개선</b><br>캘린더, 로딩 컴포넌트 제작<br>디자이너 피드백을 반영한 공통 컴포넌트 UI 개선',
      '<b>figma를 이용한 와이어프레임 작성 및 디자인 의견 제시</b>',
      '<b>반응형 웹 페이지 구현</b>',
    ],
    troubles: [
  ...
  },
  {
    id: 'egomogo',
    title: 'egomogo',
    subtitle: '명지대 인근 음식점 랜덤 추천 모바일 애플리케이션',
    ...
    tasks: [
      '<b>내 위치와 거리 설정 기능 구현</b><br>설정한 거리 이내에 있는 식당을 추천할 수 있도록 하는 기능<br>내 현재 위치와 설정한 거리에 따른 반경을 지도에 표시',
      '<b>카테고리 기능 구현</b>',
  ...
  },
  {
    ...
    tasks: [
      '<b>배송 정보 입력, 주문 조회, 이미지 업스케일링 등 13개 이상의 기능 및 화면 구현</b><br>`react-daum-postcode`를 이용한 배송지 주소 검색<br>`toast-ui/react-editor` 텍스트 에디터 사용하여 HTML 형식으로 상품 소개 글 작성<br>에디터 우측에서 html 형식으로 작성한 글에 대한 미리보기 구현<br>로그인, 로그아웃 기능 구현<br>',
